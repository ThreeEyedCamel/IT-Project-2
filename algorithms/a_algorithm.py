...
def unblocked(grid, column, row):
    ...
    return grid[row][column] != 1 #== 0 #TERESA  
...
def dest_reach(row, column, destination):
    ...
    return (row, column) == destination #teresa
...
def a_star(grid, start, end):
    ...
    if dest_reach(start[0], start[1], end): #teresa
    #if start == end:
    ...
    if not found_dest:
        raise Exception("Failed to find the destination.")
    ...
    #         if valid(next_i, next_j) and unblocked(grid, next_i, next_j): #Teresa changed the == true and kept unblocked
